...
## Reads the data from the arduino's serial port then divides them into the two strain gauges for each finger based on tab delimation 
            ...
            #print(decoded)
                    ...
                    if FTchange<measl*0.01 or FTchange<0.1:
            	        try:
         		        count+=1                    
            	        except:
         		        count=0
         		...
         		if count>4:            	        
         		        calval[count-1]=meas
         		        sgval[count-1]=SG1f[1]
                    elif FTchange>1 and count>10:
         		cv=sum(calval)/len(calval)
         		sv=sum(sgval)/len(sgval)
